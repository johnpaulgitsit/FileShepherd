...
public class FileController {
    ...
    public ResponseEntity<?> getUserFiles(HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            files.forEach(f -> System.out.println("File path: " + f.getFilePath()));
            ...
            List<Map<String, String>> fileList = files.stream()
                    .map(file -> {
                        Blob blob = storage.get(BUCKET, file.getFilePath());
                        if (blob == null) {
                            System.err.println("Missing blob for: " + file.getFilePath());
                            return null;
                        }
